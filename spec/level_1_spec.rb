...
# Unit Tests
describe Topic do
  it { should have_many(:resources) }   #add belongs to, and has many

  describe '#name' do
    it { should validate_presence_of(:name) } #presence
  end

  describe "#opinion" do
   it { should validate_presence_of(:opinion) }   #presence
    it { should_not allow_value("Too short").for(:opinion) }  #exclusion
